...
function format(tree, depth) {
	depth = depth || 0;
	var space = Array.apply(null, Array(depth)).map(function (v) {
		return '|  ';
	}).join('');
	if (typeof tree !== typeof {}) {
		...
		for (var key in tree) {
			if (tree.hasOwnProperty(key) && key !== 'type') text += space + key + ': ' + format(tree[key], depth + 1);
		}
		return text;
	}
}
...
var tokens = lex($('#input').text(), [[/not/, function (m) {
	return token('UNOP', m);
}], [/\^/, function (m) {
...
$('#tokens').text(tokens.map(function (t) {
	return t.value + ': ' + t.type;
}).join('\n'));

var tree = parse(tokens, [['S', ['block']], ['block', ['statlist']], ['block', ['cstatlist']], ['cstatlist', ['statlist', 'laststat'], function (c, l) {
	return c.concat(l);
}], ['cstatlist', ['cstatlist', 'NEWLINE']], ['statlist', ['NEWLINE'], function (n) {
	return ['begin'];
}], ['statlist', ['stat'], function (l, s) {
	return ['begin', s];
}], ['statlist', ['statlist', 'stat'], function (l, s) {
	return l.concat([s]);
}], ['statlist', ['statlist', 'NEWLINE']], ['laststat', ['return'], function (_r) {
	return null;
...
var code = (function compile(tree) {
	if (Array.isArray(tree)) return tree.map(compile);else if (tree === undefined || tree === null) return 'nil';else if (tree.type === 'STR') return '"' + tree.value;else if (tree.value !== undefined) return tree.value;else return tree;
})(tree);
...
env('print', function () {
	$('#result').append('<pre>' + Array.prototype.slice.call(arguments).join(' ') + '</pre>');
});
