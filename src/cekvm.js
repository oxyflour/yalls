...
function applyProc(proc, args, kont) {
    if (typeof(proc) === 'function')
        return [proc.apply(null, args), x => x, kont]

    var [lambda, env] = proc
    env = environment(env)
    for (var i = 1; i < lambda.length - 1; i ++)
        env.local[lambda[i]] = args[i - 1]
...
function step(exp, env, kont) {
    ...
    // set v1 a1 v2 a2
        ...
        for (var i = 1, last = null; i < exp.length; i += 2)
			last = env(exp[i], exp[i + 1])
    ...
    // fn a a
        ...
        var args = exp.slice(1).map(a => evaluate(a, env))
...
function anf(exp) {
    ...
    if (head === 'lambda') {
        ...
        if (needsWrap) {
            wrap(exp.map((e, i) => syms[i] ? null : e))
